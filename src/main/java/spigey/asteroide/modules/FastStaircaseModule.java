...
public class FastStaircaseModule extends Module {
    ...
    private void onTick(TickEvent.Post event){
        ...
        if(Registries.BLOCK.getId(mc.world.getBlockState(mc.player.getBlockPos()).getBlock()).toString().endsWith("_stairs")) 
            if(mc.player.getHorizontalFacing() == mc.world.getBlockState(mc.player.getBlockPos()).get(StairsBlock.FACING) && mc.player.input.forwardSpeed > 0) 
                mc.player.jump();
        if(asd) Modules.get().get(HighJump.class).toggle();
    }
}
